...
local Line = {}
Line.new = function(angle, radius, color,speed)
	return { angle = angle or math.random(359), --ANGLE on circle
			 radius = radius or math.random(), --[0..1]
			 color = color or math.random(4294967296), --32bit color
			 speed = speed or math.random(), --angle velocity
		   }
end
...
function draw()
	...
	for i,line in ipairs(lines)do
		...
		local a = line.angle
		local step = (line.speed * 3+1)/res --max speed 15, min speed 5
		local prevx, prevy = math.cos(a)*r, math.sin(a)*r
		for j=1,res do
			of.line(math.cos(a)*r, math.sin(a)*r, prevx, prevy)
			a = step + a
		end
	end
	of.popMatrix()
